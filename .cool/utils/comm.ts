...
/** 接口成功：code 为 1 或 200（0 为业务失败） */
export function isApiSuccess(code: number | string | null | undefined): boolean {
	if (code == null) {
		return true;
	}
...
export function getApiMessage(res: any | null, fallback: string): string {
	if (res == null) {
		return fallback;
	}
