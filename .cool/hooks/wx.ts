...
export class Wx {
		...
		if (this.isWxBrowser()) {
			request({
				...
				if (res != null) {
					...
					const payload = (body["data"] ?? res) as UTSJSONObject;
					const jsApiList = payload["jsApiList"];
					const openTagList = payload["openTagList"];
					wx.config({
						debug: config.wx.debug,
						jsApiList: jsApiList != null ? (jsApiList as any) : ["chooseWXPay"],
						appId: payload["appId"] as string,
						timestamp: payload["timestamp"] as number,
						nonceStr: payload["nonceStr"] as string,
						signature: payload["signature"] as string,
						openTagList: openTagList != null ? (openTagList as any) : undefined
					});

					// 合并配置到mpConfig
					assign(this.mpConfig, payload);
				}
			});
		}
	}
	...
	mpAuth() {
		...
		const redirect_uri = encodeURIComponent(
			`${location.origin}${location.pathname}#/pages/login/login`
		);
