...
class Size {
	...
	constructor(cb: (() => string) | null) {
		...
		// 根据全局字号配置动态计算类名（null 或 1 均为默认正常字号）
		this.ptClassName = computed(() => {
			const classText = this.className.value ?? "";
			if (config.fontSize == null || config.fontSize == 1) {
				return classText;
			}

			const name = this.names[this.getIndex()];
			return classText.replace(`-important-${name}`, "").replace(name, "");
		});
	}
	...
	getIndex = () => {
		...
		let index = this.names.findIndex((name) => {
			return classText.indexOf(name) >= 0;
		});
